...
trait CanGenerateForms
{
    protected function getResourceFormSchema(string $model): string
    {
        ...
        foreach ($schema->getColumns($table) as $column) {
            ...
            if (in_array($columnName, ['user_id', 'tenant_id', 'id', 'created_at', 'updated_at', 'deleted_at', 'status', 'slug'])) {
                continue;
            }
        ...
        foreach ($components as $componentName => $componentData) {
            ...
            $output .= '\')';
            $output .= PHP_EOL;
            $output .= '->label(static::translateForm("' . $componentName . '"))';
            $output .= PHP_EOL;
            $output .= '->placeholder(static::translateFormPlaceholder("' . $componentName . '"))';
        ...
        $output .= PHP_EOL;
        $output .= 'static::getStatusFormRadio(\'status\')->columnSpanFull(),';
        return $output;
    }
}
